<?php
/**
 * Provides the Team class for MyURY
 * @package MyURY_Core
 */
 ...
 * 
 */
class MyURY_Team extends ServiceAPI {
  /**
   * The ID of the Team
   * @var int
   */
  ...
  protected function __construct($id) {
    ...
    if (empty($result)) {
      throw new MyURYException('Team '.$id.' does not exist!', 404);
    } else {
      $this->teamid = (int)$id;
   ...
   * @return MyURY_Officer[]
   */
  public function getOfficers() {
    return MyURY_Officer::resultSetToObjArray($this->officers);
  }
  
  /**
   * Return all Users who held positions in this Team
   * @return Array {'User':User, 'from':time, 'to':time|null,
   *   'memberofficerid': int, 'position': MyURY_Officer}
   */
