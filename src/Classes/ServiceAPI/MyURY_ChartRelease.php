<?php
/**
 * Provides the MyURY_ChartRelease class for MyURY
 ...
 * @uses \Database
 */
class MyURY_ChartRelease extends ServiceAPI {
  /**
   * The singleton store for ChartRelease objects
   * @var MyURY_ChartRelease[]
   */
  ...
  /**
   * The list of IDs of MyURY_ChartRows for this chart release.
   * @var Int[]
   */
  private $chart_row_ids;

  /**
   * Constructs a new MyURY_ChartRelease from the database.
   *
   * You should generally use MyURY_ChartRelease::getInstance instead.
   *
  ...
  /**
   * Retrieves the MyURY_ChartRelease with the given numeric ID.
   *
   * @param $chart_release_id  The numeric ID of the chart release.
   * @param $chart_type        The parent chart type, if any.
   *
   ...
   *
   * @return MyURY_ChartType The chart type object.
   */
  public function getChartType() {
    if ($this->chart_type === null) {
      $this->chart_type = MyURY_ChartType::getInstance($this->chart_type_id);
    }
  ...
  public function getChartRows() {
    ...
    foreach ($this->chart_row_ids as $chart_row_id) {
      $chart_rows[] = MyURY_ChartRow::getInstance($chart_row_id, $this);
    }
    return $chart_rows;
  }
