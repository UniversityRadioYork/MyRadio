...
class MyRadio_Track extends ServiceAPI {
     ...
     * operator: Default AND. Whether results need to match one (OR) or all (AND) criteria.
     * 
    ...
    public static function findByOptions($options) {
        ...
        $result = self::$db->fetch_all('SELECT trackid, rec_track.recordid '
                . 'FROM rec_track, rec_record WHERE rec_track.recordid=rec_record.recordid '
                . $options['operator'] . ' rec_track.title ILIKE $4 || $1 || $4 '
                . $options['operator'] . ' AND rec_track.artist ILIKE $4 || $2 || $4 '
                . $options['operator'] . ' AND rec_record.title ILIKE $4 || $3 || $4
      ' . ($options['digitised'] ? ' ' . $options['operator'] . ' digitised=\'t\'' : '') . '
      ' . ($options['lastfmverified'] === true ? ' ' . $options['operator'] . ' lastfm_verified=\'t\'' : '')
                . ($options['lastfmverified'] === false ? ' ' . $options['operator'] . ' lastfm_verified=\'f\'' : '')
                . ($options['nocorrectionproposed'] === true ? ' ' . $options['operator'] . ' trackid NOT IN (
              SELECT trackid FROM public.rec_trackcorrection WHERE state=\'p\'
              )' : '')
                . ($options['clean'] != null ? ' ' . $options['operator'] . ' clean=$' . $clean_param : '')
                . ($options['custom'] !== null ? ' ' . $options['operator'] . ' ' . $options['custom'] : '')
