...
class MyURY_TracklistItem extends ServiceAPI {
  ...
  public static function getTracklistForJukebox($start = null, $end = null, $include_playout = true) {
    ...
    $result = self::$db->fetch_column('SELECT audiologid FROM tracklist.tracklist WHERE source=\'j\'
      AND timestart >= $1 AND timestart <= $2' . ($include_playout ? '' : ' AND state!=\'u\' AND state!=\'d\''), array($start, $end));
   ...
   */
  public static function getIfPlayedRecently(MyURY_Track $track, $time = 2160) {
   ...
   * @return bool
   */
  public static function getIfAlbumArtistCompliant(MyURY_Track $track, $include_queue = true, $time = null) {
    if ($time == null) {
      $time = time();
    }
