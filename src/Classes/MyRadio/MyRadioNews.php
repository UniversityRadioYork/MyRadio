...
class MyRadioNews {
    ...
    public static function getLatestNewsItem($newsfeedid, MyRadio_User $user = null) {
      ...
      ORDER BY timestamp DESC LIMIT 1', [$newsfeedid])[0], $user
                );
    }
    ...
    public static function getNewsItem($newsentryid, MyRadio_User $user = null) {
      ...
      WHERE newsentryid=$1
      AND news_feed.memberid = member.memberid', array($newsentryid));

        if (empty($news)) {
            return null;
        }

        return array_merge($news, array('seen' => $db->fetch_one('SELECT seen FROM public.member_news_feed
    ...
    public static function addItem($feedid, $content) {
        Database::getInstance()->query('INSERT INTO public.news_feed'
                . ' (feedid, memberid, content) VALUES'
                . ' ($1, $2, $3)', [$feedid, $_SESSION['memberid'], $content]);
    }
