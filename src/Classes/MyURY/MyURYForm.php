...
class MyURYForm {
  ...
  public function __construct($name, $module, $action, $params = array()) {
    ...
    foreach ($params as $k => $v) {
      ...
      if (!isset($this->$k)) throw new MyURYException('Tried to set MyURYForm parameter '.$k.' but it does not exist.');
      if (in_array($k, $this->restricted_fields)) throw new MyURYException('Tried to set MyURYForm parameter '.$k.' but it is not editable.');
      $this->$k = $v;
    }
  }
