...
var MyRadioForm = {
    ...
    if (trackFields.length > 0) {
      var trackLookup = new Bloodhound({
        datumTokenizer: function (i) {
          return Bloodhound.tokenizers.whitespace(i.title).concat(Bloodhound.tokenizers.whitespace(i.artist));
        },
        queryTokenizer: Bloodhound.tokenizers.whitespace,
        limit: 25,
        dupDetector: function (remote, local) {
  ...
  setUpArtistFields: function () {
    ...
    if (artistFields.length > 0) {
      var artistLookup = new Bloodhound({
        ...
        remote: myradio.makeURL("MyRadio", "a-findartist", {limit: 25, term: ""}) + "%QUERY" //Separated out otherwise % gets urlescaped
      });
  ...
  setUpDatePickers: function () {
     ...
     */
    $("fieldset.myradiofrm input.dateITA").datetimepicker({pickTime: false});
  },
  validate: function () {
    /**
     * Validation
  ...
  pollFileProgress: function () {
    ...
    $("#myradiofrm-file-upload-iframe").attr(
      "src",
      myradio.makeURL("MyRadio", "a-getuploadprogress", {
